...
parser = argparse.ArgumentParser('TSE_Net', add_help=False)
...
def train_fun(model, train_loader=train_loader, val_loader=val_loader, num_epoch=200,file_name=save_name):
    ...
    for epoch in range(num_epoch):
        ...
        for i, (sample) in enumerate(train_loader):
            ...
            image = torch.permute(image, (0,1,3,4,2)) # [b,c,z,h,w] -> [b,c,h,w,z]
            outputs = model(image)
...
model = create_model(input_shape=(channels, image_size[1], image_size[2], image_size[0]),num_outputs=2)
model.to(device)
train_fun(model)
