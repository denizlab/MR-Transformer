...
parser = argparse.ArgumentParser('MeT3D', add_help=False)
...
def train_fun(model, train_loader=train_loader, val_loader=val_loader, num_epoch=100,file_name=save_name):
    ...
    for epoch in range(num_epoch):
        ...
        for i, (sample) in enumerate(train_loader):
            ...
            outputs_resnet,outputs = model(image)
            loss = loss_fn(outputs_resnet, labels) + loss_fn(outputs, labels)
            loss.backward()
            optimizer.step()
...
model = MeT3D(image_size=image_size)
model.to(device)
train_fun(model)
